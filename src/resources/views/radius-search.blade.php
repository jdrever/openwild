...
<h2 class="text-start text-md-center">Search within a Radius in {{ config('core.region') }}</h2>
<div class="row mb-2">
...
<script>
	...
	const map = initialiseBasicMap('{{ config('core.region') }}',fitToRegion = false, handleResize = false, mapState = mapState)
	...
	map.on('click', function(e) {
		...
		if (map.getZoom() >= 11) {
			...
			window.location.href = "/radius/longitude/" + e.latlng.lat + "/latitude/"+ e.latlng.lat +"?mapState="+mapState;
		}
	})
